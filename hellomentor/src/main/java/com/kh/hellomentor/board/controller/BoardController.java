...
public class BoardController {
  ...
  //5-6. 자유게시판 글 삭제
    ...
    public String selectKnowledgeList(
            ...
            @RequestParam(name = "notdone", required = false) String notdone
    ) {
    	Member loginUser = (Member) session.getAttribute("loginUser");
        model.addAttribute("loginUser", loginUser);
            ...
            if ((knowledgekind != null  && keyword != null) || best!=null || notdone!=null) {
               // 검색 로직을 수행하고 결과를 처리
               totalItems = boardService.searchKnowledgeCount(knowledgekind, keyword, best, notdone); // 현재 검색된 게시글의 총 갯수 
               pageItems = boardService.searchKnowledgeList(knowledgekind, keyword, best, notdone, page, pageSize); // 현재 검색된 게시글
               pageItems2 = boardService.searchKnowledgeList2(knowledgekind, keyword, best, notdone, page, pageSize); // 현재 검색된 게시글
            } else {
               // 일반 목록을 가져옵니다
